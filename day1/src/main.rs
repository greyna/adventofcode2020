fn main() {
    ...
    for (i, first_entry) in input.iter().enumerate() {
        for second_entry in input.iter().rev().take(len - i - 1) {
            if first_entry + second_entry == 2020 {
                println!(
                    "\nPart One : found {} and {} whose sum equals 2020 and product equals {}",
                    first_entry,
                    second_entry,
    ...
    for (i, first_entry) in input.iter().enumerate() {
        for (j, second_entry) in input.iter().skip(i + 1).enumerate() {
            for third_entry in input.iter().skip(i + 1 + j + 1) {
                if first_entry + second_entry + third_entry == 2020 {
                    println!(
                        "\nPart Two : found {}, {} and {} whose sum equals 2020 and product equals {}",
                        first_entry,
                        second_entry,
                        third_entry,
