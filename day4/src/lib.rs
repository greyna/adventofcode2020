...
impl<'a> Field<'_> {
    ...
    fn is_required(&self) -> bool {
        self.field_type != FieldType::CountryID
    }

    fn is_valid(&self) -> bool {
        let res = match self.field_type {
            FieldType::BirthYear => is_year_between(self.data, 1920, 2002),
            FieldType::IssueYear => is_year_between(self.data, 2010, 2020),
            FieldType::ExpirationYear => is_year_between(self.data, 2020, 2030),
            FieldType::Height => match self.data.chars().rev().take(2).collect::<String>().as_str()
            {
                "mc" => {
                    // "cm" reverted
                    let size: u32 = self
            ...
            FieldType::HairColor => {
                self.data.len() == 7
                    ...
                    && self.data[1..].chars().all(|c| {
                        let c = c as u32;
                        (c >= 48 && c <= 57) || (c >= 97 && c <= 102)
                    })
            }
            FieldType::EyeColor => match self.data {
                "amb" | "blu" | "brn" | "gry" | "grn" | "hzl" | "oth" => true,
                _ => false,
            },
            FieldType::PassportID => self.data.len() == 9 && self.data.parse::<u32>().is_ok(),
...
fn is_year_between(input: &str, lower_bound: u32, higher_bound: u32) -> bool {
    input.len() == 4
        && input
            .parse::<u32>()
            .map_or(false, |i| i >= lower_bound && i <= higher_bound)
}
...
impl<'a> Passport<'_> {
    fn parse(input: &[&'a str]) -> Passport<'a> {
        Passport {
            fields: input.iter().map(|&s| Field::parse(s)).collect(),
        }
    }

    const NB_MAX_FIELDS: usize = 8;
    const NB_REQUIRED_FIELDS: usize = 7;
    pub fn is_valid_part_one(&self) -> bool {
        self.fields.iter().filter(|f| f.is_required()).count() == Self::NB_REQUIRED_FIELDS
    }

    pub fn is_valid_part_two(&self) -> bool {
        self.fields
            .iter()
            .filter(|f| f.is_required() && f.is_valid())
            .count()
            == Self::NB_REQUIRED_FIELDS
    }
}

pub fn parse_input<'a>(input: &[&'a str]) -> Vec<Passport<'a>> {
    input
        .iter()
        .fold(
            {
                // heuristic for capacity: around a quarter of the total number of input lines
                let mut vec = Vec::with_capacity(input.len() / 4);
                vec.push(Vec::<&str>::with_capacity(Passport::NB_MAX_FIELDS));
                vec
            },
            |mut res, &s| {
                if s.is_empty() {
                    res.push(Vec::with_capacity(Passport::NB_MAX_FIELDS));
                } else {
                    res.last_mut().unwrap().extend(s.split(' '));
                }
                res
            },
        )
        .into_iter()
        .map(|v| Passport::parse(&v))
        .collect()

    // I thought the below code would optimize by avoiding allocations, but benchmarking with criterion found otherwise!
    /*
    let mut idx = 0;
    input
        .iter()
        .fold(
            vec![Vec::<&str>::with_capacity(Passport::NB_MAX_FIELDS); input.len() / 2],
            |mut res, &s| {
                if s.is_empty() {
                    idx += 1;
                } else {
                    res[idx].extend(s.split(' '));
                }
                res
            },
        )
